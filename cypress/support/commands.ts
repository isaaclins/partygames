...
Cypress.Commands.add(
  'createGameLobby',
  (gameType: string, playerName = 'TestPlayer') => {
    cy.visit('/');
...
Cypress.Commands.add(
  'joinGameLobby',
  (lobbyId: string, playerName = 'TestPlayer2') => {
    cy.visit('/');
    cy.get('[data-cy="player-name-input"]').clear().type(playerName);
...
Cypress.Commands.add('waitForSocketConnection', () => {
  cy.window()
    .its('__socketConnected__')
    .should('equal', true, { timeout: 10000 });
});

// Two Truths and a Lie commands
Cypress.Commands.add(
  'submitTruths',
  (truth1: string, truth2: string, lie: string) => {
...
Cypress.Commands.add('checkLobbyState', (expectedPlayerCount?: number) => {
  ...
  if (expectedPlayerCount) {
    cy.get('[data-cy="player-item"]').should(
      'have.length',
      expectedPlayerCount
    );
  }
