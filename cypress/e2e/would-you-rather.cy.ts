...
describe('Would You Rather Game', () => {
  ...
  describe('Game Setup and Flow', () => {
    ...
    it('should display correct game instructions', function() {
      ...
      cy.get('[data-cy="game-instructions"]').should('contain', 'Create a Would You Rather scenario')
    })
  ...
  describe('Scenario Submission Phase', () => {
    ...
    it('should allow players to submit scenarios', function() {
      ...
      cy.get('[data-cy="waiting-message"]').should('contain', 'Waiting for other players')
    })
    ...
    it('should allow editing scenario before submission', function() {
      ...
      cy.get('[data-cy="scenario-input"]').should('have.value', 'Updated scenario')
    })
  ...
  describe('Final Results', () => {
    beforeEach(function() {
      ...
      for (let round = 1; round <= 3; round++) {
        const scenario = this.gameData.wouldYouRather.sampleScenarios[round - 1]
        cy.submitScenario(scenario)
        cy.get('[data-cy="advance-to-voting"]', { timeout: 10000 }).click()
        ...
        if (round < 3) {
          ...
          cy.get('[data-cy="advance-to-final-results"]', { timeout: 10000 }).click()
        }
      }
    })
  ...
  describe('Multi-player Simulation', () => {
    ...
    it('should rotate through different players scenarios', function() {
      ...
      cy.get('[data-cy="current-scenario-author"]').should('contain', 'Player1')
    })
  })
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper ARIA labels', function() {
      ...
      cy.get('[data-cy="submit-scenario-button"]').should('have.attr', 'aria-label')
    })

    it('should announce round changes', function() {
      cy.get('[data-cy="round-announcement"]').should('have.attr', 'role', 'status')
    })
  ...
  describe('Error Handling', () => {
    ...
    it('should validate scenario format', function() {
      cy.get('[data-cy="scenario-input"]').type('This is not a would you rather question')
      cy.get('[data-cy="submit-scenario-button"]').click()
      
      cy.get('[data-cy="format-error"]').should('be.visible')
      cy.get('[data-cy="format-error"]').should('contain', 'must be a "Would you rather" question')
    })
