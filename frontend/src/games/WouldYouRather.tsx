...
export const WouldYouRatherGame: React.FC = () => {
  ...
  const hasPlayerVotedForCurrentScenario = () => {
    ...
    return gameState.votes.some(
      v => v.voterId === playerId && v.scenarioId === gameState.currentScenario!.id
    );
  };
  ...
  if (gameState.currentPhase === 'submitting') {
      ...
      return (
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-purple-600 mb-2">Scenario Submitted!</h2>
            <p className="text-gray-600">Waiting for other players to submit their scenarios...</p>
          </div>

          <div className="bg-purple-50 p-4 rounded-lg">
            <h3 className="font-semibold mb-2">Round {gameState.currentRound} of {gameState.maxRounds}</h3>
            <div className="text-center">
              <LoadingSpinner />
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-purple-600 mb-2">Would You Rather</h2>
  ...
  if (gameState.currentPhase === 'voting') {
          ...
          <div className="bg-purple-50 p-4 rounded-lg">
            ...
            <div className="space-y-2">
              <p className="p-2 bg-white rounded border">
                <span className="font-medium text-purple-600">A:</span> {gameState.currentScenario.optionA}
              </p>
              <p className="p-2 bg-white rounded border">
                <span className="font-medium text-purple-600">B:</span> {gameState.currentScenario.optionB}
              </p>
            </div>
          </div>
          <LoadingSpinner />
          <p className="text-gray-600">
            {gameState.votingProgress?.voted || 0} / {gameState.votingProgress?.total || 0} players have voted
          </p>
        </div>
      );
    }
    ...
    const scenarioCreator = getPlayerName(gameState.currentScenario.submittedBy);
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {gameState.scenarios.map((scenario) => {
            const scenarioVotes = gameState.votes.filter(v => v.scenarioId === scenario.id);
              ...
              <div key={scenario.id} className="bg-white p-4 rounded-lg border">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 bg-purple-50 rounded">
                    <span><strong>A:</strong> {scenario.optionA}</span>
                    <span className="text-sm text-purple-600">
                      {votesA} vote{votesA !== 1 ? 's' : ''} 
                      {totalVotes > 0 && ` (${Math.round(votesA / totalVotes * 100)}%)`}
                    </span>
                  </div>
                  <div className="flex items-center justify-between p-2 bg-purple-50 rounded">
                    <span><strong>B:</strong> {scenario.optionB}</span>
                    <span className="text-sm text-purple-600">
                      {votesB} vote{votesB !== 1 ? 's' : ''}
                      {totalVotes > 0 && ` (${Math.round(votesB / totalVotes * 100)}%)`}
                    </span>
                  </div>
                </div>
              </div>
