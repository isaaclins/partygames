...
export const TwoTruthsAndALieGame: React.FC = () => {
  ...
  const handleSubmitVote = async (statementId: string, targetPlayerId: string) => {
    setIsSubmitting(true);
    try {
      await sendGameAction({
        type: 'submit_vote',
  ...
  const getCurrentTargetSubmission = () => {
    ...
    return gameState.submissions.find(s => s.playerId === gameState.currentTargetPlayer);
  };
  ...
  const hasPlayerVoted = (targetPlayerId: string) => {
    return gameState?.votes.some(v => v.voterId === playerId && v.targetPlayerId === targetPlayerId);
  };
  ...
  if (gameState.phase === 'submitting') {
      ...
      return (
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-blue-600 mb-2">Statements Submitted!</h2>
            <p className="text-gray-600">Waiting for other players to submit their statements...</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">
              {gameState.submissions.length} / {game.players.length} players have submitted
            </p>
          </div>
        </div>
      );
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-blue-600 mb-2">Two Truths and a Lie</h2>
          <p className="text-gray-600">
            Write three statements about yourself: two that are true and one that is a lie.
            Other players will try to guess which one is the lie!
          </p>
        </div>
        ...
        <Button
          ...
          disabled={isSubmitting || statements.some(s => s.trim().length === 0)}
          className="w-full"
        >
  ...
  if (gameState.phase === 'voting') {
    ...
    if (gameState.currentTargetPlayer === playerId) {
      return (
        <div className="text-center space-y-4">
          <h2 className="text-2xl font-bold text-blue-600">Other players are voting on your statements!</h2>
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="font-semibold mb-2">Your Statements:</h3>
            <ul className="space-y-2">
    ...
    if (hasVoted) {
      return (
        <div className="text-center space-y-4">
          ...
          <p className="text-gray-600">
            Waiting for other players to vote on {currentTargetName}'s statements...
          </p>
          <LoadingSpinner />
        </div>
      );
    ...
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-blue-600">Vote on {currentTargetName}'s Statements</h2>
          <p className="text-gray-600">Which statement do you think is the lie?</p>
        </div>
        ...
        <div className="space-y-3">
          {currentTargetSubmission.statements.map((statement, index) => (
            <button
              key={statement.id}
              onClick={() => handleSubmitVote(statement.id, gameState.currentTargetPlayer!)}
            ...
            >
              <span className="font-medium text-blue-600">{index + 1}.</span> {statement.text}
            </button>
          ))}
        </div>
  ...
  if (gameState.phase === 'results') {
    return (
      <div className="space-y-6">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-green-600 mb-2">Round Results!</h2>
        ...
        <div className="space-y-4">
          {gameState.submissions.map((submission) => {
            ...
            const votesForPlayer = gameState.votes.filter(v => v.targetPlayerId === submission.playerId);
            
            return (
              <div key={submission.playerId} className="bg-white p-4 rounded-lg border">
                <h3 className="font-semibold text-lg mb-3">{playerName}'s Statements</h3>
                <div className="space-y-2">
                  {submission.statements.map((statement, index) => {
                    const votesForStatement = votesForPlayer.filter(v => v.selectedStatementId === statement.id);
                    ...
                    return (
                      <div 
                        ...
                        className={`p-3 rounded border-l-4 ${
                          isLie ? 'border-red-500 bg-red-50' : 'border-green-500 bg-green-50'
                        }`}
                      >
                        <div className="flex justify-between items-start">
                          <span className="flex-1">
                            ...
                            {isLie && <span className="text-red-600 font-bold ml-2">(LIE)</span>}
                          </span>
                          <span className="text-sm text-gray-600">
                            {votesForStatement.length} vote{votesForStatement.length !== 1 ? 's' : ''}
                          </span>
                        </div>
                      </div>
                    );
            ...
            {Object.entries(gameState.scores)
              ...
              .map(([playerId, score]) => (
                <div key={playerId} className="flex justify-between items-center">
                  ...
                  <span className="text-blue-600 font-bold">{score} points</span>
                </div>
              ))}
          </div>
        </div>
