...
describe('GameStore', () => {
  ...
  describe('Player Management', () => {
    ...
    test('removePlayer removes the correct player', () => {
      ...
      expect(result.current.players.length).toBe(2);
      expect(
        result.current.players.find((p) => p.id === 'player-2')
      ).toBeUndefined();
      expect(
        result.current.players.find((p) => p.id === 'player-1')
      ).toBeDefined();
      expect(
        result.current.players.find((p) => p.id === 'player-3')
      ).toBeDefined();
    });
...
describe('GameStore Selectors', () => {
  ...
  describe('useGameSelectors', () => {
    ...
    test('calculates connectedPlayersCount correctly', () => {
      ...
      act(() => {
        storeResult.current.updatePlayers([
          mockPlayer1,
          mockPlayer2,
          mockPlayer3,
        ]);
