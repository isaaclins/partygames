...
describe('Input Component', () => {
  ...
  describe('Variants', () => {
    test('renders default variant by default', () => {
      ...
      const input = screen.getByTestId('input');
      expect(input).toHaveClass('border', 'border-slate-300', 'rounded-lg', 'bg-white');
    });

    test('renders outline variant', () => {
      render(<Input variant="outline" data-testid="input" />);
      const input = screen.getByTestId('input');
      expect(input).toHaveClass('border-2', 'border-slate-300', 'rounded-lg', 'bg-transparent');
    });
  });
  ...
  describe('Icons', () => {
    ...
    test('icon containers have proper positioning', () => {
      ...
      expect(leftIconContainer).toHaveClass('absolute', 'left-3', 'top-1/2', 'transform', '-translate-y-1/2');
      expect(rightIconContainer).toHaveClass('absolute', 'right-3', 'top-1/2', 'transform', '-translate-y-1/2');
    });
  });
  ...
  describe('Layout and Container', () => {
    ...
    test('maintains proper spacing between elements', () => {
      const { container } = render(
        <Input label="Test" helperText="Helper" />
      );
